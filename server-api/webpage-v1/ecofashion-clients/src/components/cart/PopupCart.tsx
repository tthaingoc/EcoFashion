      ...
      <div className="p-5">
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="space-y-4 max-h-80 overflow-y-auto">
                         {cartItems.map((item) => (
               <div key={item.id} className="flex items-center gap-3 border-b pb-3 last:border-b-0 relative group">
                 <img src={item.image} alt={item.name} className="w-16 h-16 object-cover rounded" />
                 <div className="flex-1">
                   <div className="flex items-center gap-2">
                     <span className="text-xs text-green-500 font-bold">•</span>
                     <span className="font-medium">{item.name}</span>
                     <span className="text-xs text-gray-400 ml-1">({item.type === 'material' ? 'Nguyên liệu' : 'Sản phẩm'})</span>
                   </div>
                   <div className="text-sm text-gray-500">{item.price.toLocaleString('vi-VN')} ₫/{item.unit || 'cái'}</div>
                   <div className="flex items-center gap-2 mt-1">
                     <button onClick={() => decreaseQuantity(item.id)} className="w-6 h-6 flex items-center justify-center bg-gray-100 rounded hover:bg-red-100 text-red-500">-</button>
                     <span className="px-2">{item.quantity} {item.unit || 'cái'}</span>
                     <button onClick={() => increaseQuantity(item.id)} className="w-6 h-6 flex items-center justify-center bg-gray-100 rounded hover:bg-green-100 text-green-600">+</button>
                   </div>
                 </div>
                 {/* Nút xóa */}
                 <button 
           ...
           <span className="font-bold text-green-700">{subtotal.toLocaleString('vi-VN')} ₫</span>
         </div>
