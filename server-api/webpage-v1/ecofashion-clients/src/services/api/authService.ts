...
export interface SupplierData {
  ...
  userId: number;
  supplierName?: string;
  avatarUrl?: string; // Avatar from Supplier table
  bio?: string;
...
export class AuthService {
  ...
  static async login(email: string, password: string): Promise<AuthResponse> {
    try {
      ...
      // If user is supplier, fetch and store supplier data
      if (authData.user.role === 'supplier') {
        await AuthService.fetchAndStoreSupplierData(authData.user.userId);
      }
  ...
  /**
   * Fetch and store supplier data for supplier users
   */
  static async fetchAndStoreSupplierData(userId: number): Promise<void> {
    try {
      // Use the correct endpoint: /Supplier/profile (authenticated endpoint)
      const response = await apiClient.get<BaseApiResponse<SupplierData>>(
        `/Supplier/profile`
      );

      const supplierData = handleApiResponse(response);
      
      // Store supplier data with supplierAvatarUrl for compatibility
      const supplierInfo = {
        ...supplierData,
        supplierAvatarUrl: supplierData.avatarUrl // Map avatarUrl to supplierAvatarUrl
      };
