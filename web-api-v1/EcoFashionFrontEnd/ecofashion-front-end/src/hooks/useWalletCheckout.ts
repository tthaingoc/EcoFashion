...
// Truyền thêm addressId vào mutation
export const usePayOrderWithWallet = () => {
 const queryClient = useQueryClient();
 return useMutation({
   mutationFn: async ({ orderId, addressId }: { orderId: number; addressId: number }) => {
...
export const usePayGroupWithWallet = () => {
 const queryClient = useQueryClient();
 return useMutation({
   mutationFn: async ({ orderGroupId, addressId }: { orderGroupId: string; addressId: number }) => {
