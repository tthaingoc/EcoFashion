...
namespace EcoFashionBackEnd.Services
{
    public class FlexibleCheckoutService
    {
        ...
        public async Task<CheckoutSessionDto> CreateCheckoutSessionFromCartAsync(int userId, string? shippingAddress = null, int? addressId = null, List<int>? selectedCartItemIds = null)
        {
            ...
            var cartItemsToProcess = activeCart.Items.AsQueryable();
            if (selectedCartItemIds != null && selectedCartItemIds.Any())
            {
                cartItemsToProcess = cartItemsToProcess.Where(item => selectedCartItemIds.Contains(item.CartItemId));
            }

            var filteredItems = cartItemsToProcess.ToList();
            if (!filteredItems.Any())
                throw new ArgumentException("No valid cart items found for checkout");

            var request = new CreateCheckoutSessionRequest
            {
                ShippingAddress = shippingAddress,
                AddressId = addressId,
                Items = filteredItems.Select(item => new CheckoutSessionItemRequest
                {
