...
interface FlexibleCheckoutCartProps {
  ...
  onPaymentSuccess?: (orderGroupId: string) => void;
}
...
const FlexibleCheckoutCart: React.FC<FlexibleCheckoutCartProps> = ({ 
  ...
  const { items: cartItems, syncFromServer } = useCartStore();
  ...
  const createSessionFromCart = async () => {
    try {
      setLoading(true);
      // Sync cart from server first
      await syncFromServer();
      
      // Get selectedCartItemIds if groupSellerId is specified
      let selectedCartItemIds: number[] | undefined;
      if (groupSellerId) {
        const filteredItems = cartItems.filter(item => item.sellerId === groupSellerId);
        selectedCartItemIds = filteredItems.map(item => Number(item.id));
      }
