...
const AddressFormModal: React.FC<AddressFormModalProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    fullName: address?.fullName || (user?.fullName || ''), // Use address fullName or fallback to user's name
  ...
  useEffect(() => {
    if (address) {
      // Editing existing address - populate all fields from address, fallback to user name if no fullName
      setFormData({
        fullName: address.fullName || (user?.fullName || ''), // Use address fullName or fallback to user's name
      ...
      setAllowEditName(true); // Always allow editing when editing existing address
    } else if (user) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submitData = {
      // Only include fields that backend UserAddress supports
      city: formData.city,
      ...
      // Note: fullName không được gửi vì backend UserAddress table không có field này
    };
  ...
  const handleProvinceChange = (provinceName: string, provinceCode: string) => {
  ...
  const handleDistrictChange = (districtName: string, districtCode: string) => {
    handleChange('district', districtName);
    // Clear ward when district changes  
    handleChange('ward', '');
  };

  const handleWardChange = (wardName: string, wardCode: string) => {
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                {!address && allowEditName && (
                  <p className="text-xs text-yellow-600 mt-1">
                    💡 Thay đổi này chỉ áp dụng cho địa chỉ này, không ảnh hưởng đến tài khoản.
                  </p>
                )}
                {address && (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Số điện thoại {!address && user?.phone && '(từ thông tin tài khoản)'}
                </label>
                <input
                  type="tel"
                ...
                />
                {!address && user?.phone && formData.personalPhoneNumber === user.phone && (
                  <p className="text-xs text-gray-500 mt-1">
                    Số điện thoại từ tài khoản. Bạn có thể thay đổi cho địa chỉ này.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="flex gap-3 mt-8 pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={isLoading || !formData.city || !formData.district || !formData.addressLine}
