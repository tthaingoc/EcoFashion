...
const CheckoutTailwind: React.FC = () => {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] =
    useState<"wallet">("wallet"); // Phương thức thanh toán đã chọn
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        if (existingOrderId) {
          ...
          const orderData = await ordersService.getById(
            parseInt(existingOrderId)
          );
          ...
          wizard.start({
            ...
            totalAmount: resp.orders.reduce(
          ...
          const selectedItemsStr = localStorage.getItem(
            "selectedItemsForCheckout"
          );
          console.log(
            "CheckoutTailwind bootstrap - selectedItemsStr:",
            selectedItemsStr
          );
          ...
          if (!selectedItemsStr) {
            ...
            console.log(
              "No selectedItemsForCheckout found, redirecting to /checkout/confirm"
            );
          ...
          setTotalAmount(
            sessionResp.totalAmount ||
            sessionResp.orders.reduce(
          ...
          if (sessionResp.orderIds && sessionResp.orderIds.length > 0) {
            sessionStorage.setItem(
              "checkoutOrderIds",
              JSON.stringify(sessionResp.orderIds)
            );
            sessionStorage.setItem(
              "checkoutOrderGroupId",
              sessionResp.orderGroupId
            );
            console.log("Saved orderIds to session:", sessionResp.orderIds);
            console.log(
              "Saved orderGroupId to session:",
              sessionResp.orderGroupId
            );
          }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {wizard.orders.length > 0
              ? `Thanh toán cho ${wizard.orders.length} đơn hàng từ ${wizard.orders.length
              } người bán khác nhau - Tổng: ${orderTotal.toLocaleString(
                "vi-VN"
              )} VND`
              : `${selectedCartItems.length
              } sản phẩm đã chọn - Tổng: ${orderTotal.toLocaleString(
                "vi-VN"
              )} VND`}
          </p>
          ...
          {wizard.orders.length > 0 && (
            <div className="mt-2 text-sm text-gray-500">
              <span>
                Mã đơn hàng:{" "}
                {wizard.orders.map((o) => `#ĐH${o.orderId}`).join(", ")}
              </span>
            </div>
          )}
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 sticky top-8">
              ...
              <button
                ...
                disabled={
                  isProcessing || !selectedAddress || walletBalance < orderTotal
                }
              ...
              >
                {isProcessing ? (
                  ...
                  <>
                    ...
                    {wizard.orders.length > 1
                      ? `Thanh toán ${wizard.orders.length
                      } đơn hàng - ${orderTotal.toLocaleString("vi-VN")} VND`
              ...
              {wizard.orders.length > 1 && (
                <div className="mt-2 text-xs text-gray-500 text-center">
                  Bạn sẽ thanh toán một lần cho tất cả {wizard.orders.length}{" "}
                  đơn hàng
                </div>
              )}
            </div>
