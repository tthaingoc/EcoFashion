...
export default function DesignerProfile() {
  ...
  const displayName =
    combinedDesigner?.designerName ||
    combinedDesigner?.user?.fullName ||
    "Nhà thiết kế";
  const displayEmail =
    combinedDesigner?.email || combinedDesigner?.user?.email || "Chưa cập nhật";
  const displayPhone =
    combinedDesigner?.phoneNumber ||
    combinedDesigner?.user?.phone ||
    "Chưa cập nhật";
  const displayAddress = combinedDesigner?.address || "Chưa cập nhật";
  const displayBio = combinedDesigner?.bio || "Nhà thiết kế chuyên nghiệp";
          ...
          <div className="max-w-[1720px] mx-auto flex items-start gap-8 md:gap-12 2xl:gap-24 flex-col md:flex-row my-profile-navtab">
            ...
            <div className="w-full md:w-auto md:flex-1 overflow-auto">
              <div className="w-full max-w-[951px] bg-[#F8F8F9] dark:bg-dark-secondary p-5 sm:p-8 lg:p-[50px]">
                ...
                <span className="leading-none mt-3 block">{displayBio}</span>
                <div
