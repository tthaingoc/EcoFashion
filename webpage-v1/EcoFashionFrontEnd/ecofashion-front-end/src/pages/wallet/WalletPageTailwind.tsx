  ...
  useEffect(() => {
    ...
    if (success === "deposit") {
      const result = {
        ...
        message: `Nạp tiền thành công! Số tiền ${
          amount
            ? new Intl.NumberFormat("vi-VN").format(parseInt(amount)) + " VND"
            : ""
      ...
      setPaymentResult({
        status: "success",
        message: `Rút tiền thành công! Số tiền ${
          amount
            ? new Intl.NumberFormat("vi-VN").format(parseInt(amount)) + " VND"
            : ""
        } đã được chuyển về tài khoản ngân hàng.`,
        transactionId: transactionId ? parseInt(transactionId) : undefined,
      });
      setShowResultModal(true);
      refreshWallet();
